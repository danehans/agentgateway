...
vSR supports multiple response-cache backends, including its default in-memory
...
See the [vSR response-cache documentation](https://vllm-sr.ai/docs/tutorials/plugin/response-cache/),
...
agentgateway
  |
  +----> vSR ExtProc ----> Redis response-cache lookup
  |                          |
  |                          `---- hit: cached response
  |
...
The route uses `response_cache` in `semantic` mode with `scope: global` because
the mock product answers are identical for every caller. The threshold of 0.70
is for this demonstration only. See the [shared overview](../README.md).
...
- Configure Redis with a server certificate, private key, and trusted CA as
  ...
  TLS in the response-cache Redis connection. See the vSR [Redis cache
  configuration](https://github.com/vllm-project/semantic-router/blob/main/config/runtime/response-cache/redis.yaml).
...
```bash
kind create cluster \
  --config examples/llm-semantic-routing/response-cache/kubernetes/kind-config.yaml
```
...
```bash
source examples/llm-semantic-routing/response-cache/kubernetes/versions.env
...
```bash
kubectl apply \
  -f examples/llm-semantic-routing/response-cache/kubernetes/gateway.yaml
...
```bash
kubectl apply \
  -f examples/llm-semantic-routing/response-cache/kubernetes/redis.yaml
...
The setup script creates a ConfigMap from [the shared Python backend](../shared/support-backend.py).
...
```bash
examples/llm-semantic-routing/response-cache/kubernetes/configure.sh
...
examples require ExtProc streaming changes absent from v0.3.0. Keep the
`latest` defaults; record the resolved image digest when reporting test results.
...
```bash
kubectl apply \
  -f examples/llm-semantic-routing/response-cache/kubernetes/agentgateway-routing.yaml
...
```bash
examples/llm-semantic-routing/response-cache/kubernetes/verify.sh
```

The script resets only the example's mock counter and cache keys in its dedicated Redis instance, then
asserts:
...
cache hit. It also discovers and inspects the response-cache Redis Search indexes.
Current vSR derives physical index names and key prefixes from the embedding
identity, so they can differ from the configured `semantic_cache_idx` name.
...
deletes the original pod, and verifies that the original request is still a cache hit:

```bash
examples/llm-semantic-routing/response-cache/kubernetes/verify.sh --shared-vsr
```
...
```bash
examples/llm-semantic-routing/response-cache/kubernetes/verify.sh --redis-restart
```
...
kubectl delete \
  -f examples/llm-semantic-routing/response-cache/kubernetes/support-backend.yaml
kubectl delete \
  -f examples/llm-semantic-routing/response-cache/kubernetes/redis.yaml
kubectl delete \
  -f examples/llm-semantic-routing/response-cache/kubernetes/gateway.yaml
